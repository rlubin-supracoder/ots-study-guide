...
const staffList=process.env.TETHER_STAFF_EMAILS||process.env.TETHER_STAFF_EMAIL;
if(!process.env.CLOUDFLARE_ACCESS_TOKEN_FILE||!staffList)throw new Error('Set CLOUDFLARE_ACCESS_TOKEN_FILE and TETHER_STAFF_EMAILS.');
const token=(await readFile(process.env.CLOUDFLARE_ACCESS_TOKEN_FILE,'utf8')).trim();
const config=JSON.parse(await readFile(new URL('../wrangler.jsonc',import.meta.url),'utf8'));
const domain=new URL(config.vars.APP_ORIGIN).hostname,staffDomain=domain+'/staff',emails=[...new Set(staffList.split(',').map(value=>value.trim().toLowerCase()))].sort();
if(emails.some(value=>! /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value)))throw new Error('Invalid staff email list.');
...
if(!process.argv.includes('--apply')){console.log(JSON.stringify({current:app.domain,target:staffDomain,policyCount:policies.length,approvedEmailCount:emails.length,mode:'review'}));process.exit(0);}
// Restrict staff identity first. Deploy the password-protected Worker before applying this script.
await api('apps/'+app.id+'/policies/'+policies[0].id,'PUT',{name:'Tether approved staff',decision:'allow',precedence:1,include,exclude:[],require:[]});
...
const actualEmails=verifiedPolicies[0]?.include?.map(rule=>rule.email?.email).sort();
if(verified.domain!==staffDomain||verified.aud!==config.vars.ACCESS_AUD||verifiedPolicies.length!==1||verifiedPolicies[0].decision!=='allow'||JSON.stringify(actualEmails)!==JSON.stringify(emails))throw new Error('Configuration verification failed.');
console.log(JSON.stringify({appId:app.id,domain:verified.domain,audienceUnchanged:true,approvedEmailCount:emails.length,configured:true}));
