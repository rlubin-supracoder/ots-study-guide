...
input.value = 'Valor'; submit({ preventDefault() {} });
assert.equal(events.at(-1), 'valor:open', 'Valor opens the separate check-in app');
assert.equal(unlocked, false, 'Valor must not unlock the trainer');
assert.equal(storage.size, 0, 'Valor must not persist a trainer session');
